...
describe("beş aksiyonlu kaynak bağlantı motoru",()=>{
  ...
  it("Bakara 153 Elmalılı tefsir resmî eser sayfası",()=>{const link=buildSourceLink({sourceId:"elmalili",contentType:"tafsir",surahNumber:2,ayahStart:153});expect(link.url).toContain("ekitap.yek.gov.tr/urun/hak-dini-kur-an-dili");expect(link.url).toBe(link.generalFallbackUrl);expectVerified(link);});
